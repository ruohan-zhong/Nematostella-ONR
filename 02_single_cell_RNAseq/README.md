# Single-cell RNA-seq

Code for processing and downstream analysis of the adult oral-region single-cell RNA-seq dataset.

Raw and processed data are available through GEO under accession **GSE316974**.

Reference genome is *Nematostella vectensis* jaNemVect1.1 (GCF_932526225.1).

`scRNAseq_processing.R` generates the Seurat object used for downstream analysis.

Large sequencing files and Seurat objects are deposited in GEO rather than duplicated here.
